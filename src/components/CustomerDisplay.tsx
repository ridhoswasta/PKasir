...
import { Coffee, ShoppingCart, CheckCircle2, Maximize, Minimize, QrCode, Smartphone, BookOpen } from 'lucide-react';
...
export function CustomerDisplay() {
  ...
  return (
    <div className="absolute inset-0 bg-background text-foreground flex flex-col">
      {/* Fullscreen toggle — subtle, top-right */}
      <button
      ...
      {/* Header */}
      <div className="px-10 py-8 border-b border-border/70 flex items-center gap-5 bg-background">
        {state.logo ? (
      ...
      {/* Cart area */}
      <div ref={cartScrollRef} className="flex-1 overflow-y-auto px-10 py-6">
        {state.cart.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-muted-foreground/70 gap-6">
            <ShoppingCart className="w-32 h-32 opacity-30" strokeWidth={1} />
            <div className="text-center">
              <p className="text-3xl font-bold text-muted-foreground">Belum ada pesanan</p>
              <p className="text-muted-foreground/70 mt-2">Menunggu kasir menambahkan item...</p>
            </div>
          </div>
        ) : (
